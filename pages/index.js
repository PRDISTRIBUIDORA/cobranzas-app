  ...
  return <div style={{minHeight:"100vh",background:"#080b10",display:"flex",alignItems:"center",justifyContent:"center"}}>
    <div style={{...S.card,padding:28,width:320}}>
      ...
      <Field label="Contraseña">
        <input style={S.input} type="password" value={pw} onChange={e=>setPw(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handle()} placeholder="••••••••" autoFocus />
      </Field>
...
export default function Home() {
  ...
  if (showLogin) return <Login onLogin={()=>{ setIsAdmin(true); setShowLogin(false); setTab("deudores"); }} onCancel={()=>setShowLogin(false)} />;
      ...
      <div style={{background:"rgba(13,17,23,.95)",borderBottom:"1px solid rgba(255,255,255,.07)",padding:"0 16px",position:"sticky",top:0,zIndex:40}}>
        <div style={{maxWidth:1100,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",height:54,flexWrap:"wrap",gap:6}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:3,flexWrap:"wrap"}}>
            ...
            {isAdmin
              ? <button onClick={()=>{setIsAdmin(false);setTab("deudores");}} style={{...S.btnGhost,fontSize:11,padding:"5px 10px",marginLeft:6}}>Salir</button>
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:"20px 16px"}}>
        <div style={{marginBottom:18}}>
          ...
          <p style={{fontSize:12,color:"#475569",margin:0}}>
            {tab==="deudores" && "Clientes por antigüedad de deuda — registrá cobros desde acá"}
            {tab==="visitas"  && "Agenda y seguimiento de visitas a clientes"}
            {tab==="clientes" && "Lista de clientes activos"}
            {tab==="importar" && "Importar Excel desde tu sistema de gestión"}
            {tab==="resumen"  && "Resumen semanal por vendedor — exportable a Excel"}
          </p>
        </div>
